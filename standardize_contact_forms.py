#!/usr/bin/env python3
"""
Site-wide Contact Form Standardization Script for Golden Grove Cleaning
Ensures 100% of pages have the identical, official LeadConnector contact form (Qw27XO1tcXra7A4hA2hy),
identical 2-column layout (Form + Direct Contact Info + Phone + Hours + Trust Badges),
...
def get_standard_quote_section(city_name="Texas"):
    return f"""        <!-- MODERN QUOTE & CONTACT FORM -->
        <section id="quote"
            ...
            <div class="max-w-6xl mx-auto px-5 sm:px-8 lg:px-12 relative">
                <div class="text-center mb-12 md:mb-16 animate-fade-in-up">
                    <h2 class="text-4xl sm:text-5xl font-bold text-gray-800 tracking-tight">
                        Get Your <span class="text-orange-600">Free Quote</span> in {city_name}
                    </h2>
                    ...
                    <div class="space-y-8">
                        <div
                            ...
                            <div class="mt-8 pt-8 border-t border-gray-100">
                                <a href="/book-now/" class="w-full py-4 px-6 bg-brand-green hover:bg-brand-green/90 text-white text-center text-lg font-bold rounded-2xl transition-all shadow-md flex items-center justify-center gap-3">
                        ...
                        <div
                            ...
                            <p class="text-sm text-gray-600">
                                Fully bonded, insured, and background-checked cleaning pros serving {city_name} with our 100% 24-hour satisfaction warranty.
                            </p>
                        </div>
                    </div>
                </div>
...
print("Auditing & Standardizing Contact Forms Across Entire Site...")
updated_forms = 0
total_checked = 0
...
for dirpath, dirnames, filenames in os.walk(ROOT):
    ...
    for fname in filenames:
        ...
        # Don't overwrite book-now form (which is the interactive BookingKoala widget)
        ...
        # Check if page has #quote section or is missing standard LeadConnector embed
        if '<section id="quote"' in content:
            # Check if it already has the exact LeadConnector form
            if 'Qw27XO1tcXra7A4hA2hy' not in content or '<!-- MODERN QUOTE & CONTACT FORM -->' not in content:
                # Replace existing section #quote
                content = re.sub(
                    r'<section\s+id=[\"\']quote[\"\'].*?</section>',
                    get_standard_quote_section(city_name),
                    content,
            ...
            # If page doesn't have a #quote section at all, inject the standard quote section right before the CTA banner or footer
            std_section = get_standard_quote_section(city_name)
...
print(f"\n✅ Form Standardization Complete across {total_checked:,} pages!")
print(f"  Standardized/Updated Forms: {updated_forms:,}")
